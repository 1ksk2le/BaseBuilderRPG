...
namespace BaseBuilderRPG.Content
{
    public static class SpriteBatchExtensions
    {
        ...
        public static void DrawStringWithOutline(this SpriteBatch spriteBatch, SpriteFont font, string text, Vector2 position, Color outlineColor, float outlineThickness, float layerDepth, float scale)
        {
            for (float i = 0; i < 360; i += 45)
            {
                float angle = MathHelper.ToRadians(i);
                Vector2 offset = new Vector2((float)Math.Cos(angle), (float)Math.Sin(angle)) * outlineThickness * scale;
